...
const VMAP = {
  ...
  // acquisition method (work)
  '기증':['기증','Donation'], '구입':['구입','Purchase'], '관리전환':['관리전환','Management Transfer'],
  // program type
...
export function seriesLabel(v){ if(getLang()!=='en' || !v) return v; return String(v).split(',').map(s=>{const t=s.trim(); return SERIES_EN[t]||t;}).join(', '); }  // 콤마 병기 시리즈도 개별 번역
